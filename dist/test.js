define(["require", "exports", "./recurso"], function (require, exports, recurso_1) {
    "use strict";
    ...
    const fibonacci = recurso_1.recurso({
        base: [0, 1],
        recurrence: ([x, y]) => x + y,
    });
    const factorial = recurso_1.recurso({
        base: [1],
        recurrence: ([x], n) => n * x,
    });
    const sum = arr => arr.reduce((sumAcc, c) => sumAcc + c, 0);
    const getKbonacci = k => recurso_1.recurso({
        base: [...new Array(k)].map((_, i) => i),
        recurrence: sum,
    });
    const triFibonacci = getKbonacci(3);
    const quadFibonacci = getKbonacci(4);
    const explicitFibonacci = recurso_1.recurso({
        base: [0, 1],
        next: [n => n - 2, n => n - 1],
        recurrence: ([x, y]) => x + y,
    });
    const implicitExplicitFibonacci = recurso_1.recurso({
        base: [0, 1],
        next: [n => n - 1],
        recurrence: ([x, y]) => x + y,
    });
    const explicitFactorial = recurso_1.recurso({
        base: [1],
        next: [n => n - 1],
        recurrence: ([x], n) => n * x,
    });
    const numDerangements = recurso_1.recurso({
        base: [1, 0],
        recurrence: ([x, y], n) => (n - 1) * (x + y),
    });
    const explicitNumDerangements = recurso_1.recurso({
        base: [1, 0],
        next: [(n) => n - 1, (n) => n - 2],
        recurrence: ([x, y], n) => (n - 1) * (x + y),
    });
    const subsets = l => recurso_1.recursoList(l, {
        base: [[[]]],
    ...
    const and = l => recurso_1.recursoList(l, {
        base: x => {
            if (!x)
                return false;
        },
    ...
    const makeChange = (coins, target) => recurso_1.recurso({
        base: ({ coins, target }) => {
            if (!coins.length)
                return 0;
            if (!target)
    ...
    const binarySearch = (arr, target) => recurso_1.recurso({
