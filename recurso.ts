...
const getGetBaseCase = <T>(base: (...args: any[]) => T | T[], ordering) => {
  ...
  if (ordering && base instanceof Array) {
    ...
    return recursiveCase => base[[...new Array(base.length)].map((_, i) => i).filter(i => firstCases[i] === recursiveCase)[0]] //error handling
  }
...
const getExplicitBaseFromOrdering = ({ base, ordering = makeRangeIterator(), offset = 0 } = { base: undefined }) => {
  ...
  for (let internalCase of ordering) {
    if (i > MaxIterability) { throw 'base case overflow' }
    let nextValue = access(base, internalCase)
    if (i === offset && nextValue === undefined) { throw 'no base cases for tail optimization' }
    if (i > offset && nextValue === undefined) { break }
    if (i >= offset) {
...
const recurso = ({
    base = undefined as (...args) => any | any[],
    ordering = undefined,
