type Mappable<T, ImmutableOrdinal> =
  | ((o: ImmutableOrdinal) => T)
  | Iterable<T>
  | Map<ImmutableOrdinal, T>
  | T[]
  | IterableIterator<T>
type Orderable<T> =
  | ((o: number) => T)
...
type Next<ImmutableOrdinal> =
  | ((o: ImmutableOrdinal) => ImmutableOrdinal)[]
  | ((o: ImmutableOrdinal) => ImmutableOrdinal[])
...
const getIteratedNextFunction = <ImmutableOrdinal>(
...
const getDefaultTuplicity = <ImmutableOrdinal, T>(
  next: Next<ImmutableOrdinal | number>,
...
const getGetBaseCase = <T, ImmutableOrdinal>(
  base: Mappable<T, ImmutableOrdinal>,
...
const getExplicitBaseFromOrdering = <T, ImmutableOrdinal>({
  base,
...
const getIsSinglePassTailable = <T, ImmutableOrdinal>(
  base: Mappable<T, ImmutableOrdinal>,
  ordering: IterableIterator<T>
...
// export
const robo = <T, ImmutableOrdinal>({
  base,
  ordering,
  memoize,
  next = DefaultNext,
  recurrence = DefaultRecurrence,
  tuplicity = getDefaultTuplicity<ImmutableOrdinal, T>(next, base),
  offset = 0,
...
}: // = getIsIterable(base) ? makeOffsetIterator(makeRangeIterator(), offset) : undefined // iterable or function
{
  ...
  ordering?: Orderable<T>
  next?: Next<ImmutableOrdinal>
  recurrence?: (recursiveCases: T[], o: ImmutableOrdinal | number) => T
  tuplicity?: number
  offset?: number
  memoize?:
    | ((o: ImmutableOrdinal) => string | number)
    | ((o: any) => string | number)[]
    | true
  ...
  if (shouldBeSinglePassOptimized) {
    ...
    debugger
    interface MetaOrdinal<T, ImmutableOrdinal> {
      accs: T[]
      outerCase: ImmutableOrdinal
      innerCase?: ImmutableOrdinal
    }
    return robo<T, MetaOrdinal<T, ImmutableOrdinal>>({
      base: ({ accs, outerCase }) => {
